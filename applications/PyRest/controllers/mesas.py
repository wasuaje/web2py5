...
def handle():
	...
	if len(request.args) > 0:
		...
		registro = 0
	form = SQLFORM(db[tabla], registro, deletable=True,  submit_button='Enviar' ,    delete_label='Click para borrar:',  next=URL(c='mesas',  f='handle'), \
...
def getdata():
	...
	if searching:
		query=db( (eval(searchquery)) & (db.cf_mesa.id > 0) ).select()
	else:				
		query=db(db.cf_mesa.id > 0).select( limitby=( start, limit) )
		
	for row in query:
	#for row in  db(db.fc_cobro.cf_proveedor_id==db.cf_proveedor.id).select():
		rw={}
		rw['id']=row.id
		rw['cell']=[row.id,row.codigo,row.nombre]
		resp['rows'].append(rw)
