...
def handle():
	...
	if len(request.args) > 0:
		...
		registro = 0
	form = SQLFORM(db[tabla], registro, deletable=True,  submit_button='Enviar' ,    delete_label='Click para borrar:',  next=URL(c='empresa',  f='handle'), \
...
def getdata():
	...
	#for row in  db(db.fc_cobro.cf_proveedor_id==db.cf_proveedor.id).select():
		rw={}
		rw['id']=row.id
		rw['cell']=[row.id,row.razon_social,row.rif]
		resp['rows'].append(rw)
