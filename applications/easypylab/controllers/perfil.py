...
def index():	
	...
	myform=FORM(TABLE(TR(B('Cod.'),B('Nombre'),B('P.Unit'),B('P.Seg.'),B('P.Emp'),B('PE1'),B('PE2')),
					...
					INPUT(_type='button',_value='+',_id='adp',_name='adp',_onclick="ajax('subhandle', \
											['codigo','nombre','punit','pseg','pemp','pe1','pe2','ad'],'result')"),
					INPUT(_type='button',_value='-',_id='delp',_name='delp')
					  )	\
				     ) \
			   )
	
	query = db( (db[tabla].id > 0) & (db[tabla].es_perfil == None)  & (db[tabla].examen_id == None) ).select()
	
	myform2=FORM(TR("Seleccione Examen: ", \
          SELECT(_name='examenes', _id='examenes',  \
          *[OPTION(query[i].nombre, _value=str(query[i].id)) for i in range(len(query))])), \
          TR(INPUT(_type='button',_value='+',_id='adex',_name='adex'),
					INPUT(_type='button',_value='-',_id='delex',_name='delex')
          	)
          )
	
    

	return dict(myform=myform, myform2=myform2)
...
def subhandle():	
	...
	if request.vars.action=='del':
		...
		examen_id=db.cf_examen.insert(codigo=codigo,nombre=nombre,precio_fin=punit,precio_seg=pseg, \
								precio_emp=pemp,precio_esp1=pe1,precio_esp2=pe2,es_perfil=1)				
...
def subhandle2():	
	scr="""			
		...
		""" 
	tabla='cf_examen'
	pe_id=request.vars.id_perfil	#id del perfil al que pertenece
	ex_id=request.vars.id_examen	#id del examen que estoy duplicando
	...
	if request.vars.action=='del':
		...
		#examen_id=db.cf_examen.insert(codigo=codigo,nombre=nombre,precio_fin=punit,precio_seg=pseg, \
		#						precio_emp=pemp,precio_esp1=pe1,precio_esp2=pe2,es_perfil=1)				
		qry="insert into %s (codigo,nombre,examen_id) select codigo,nombre,%s from %s where id = %s;" \
							 % (tabla,pe_id,tabla,ex_id)
		#print qry
		#print db._lastsql
		examen_id = db.executesql(qry)
...
def getdata():
	...
	if searching:
		query=db( (eval(searchquery)) & (db[tabla].es_perfil != None)  ).select()
	else:				
		query=db( (db[tabla].id > 0) & (db[tabla].es_perfil != None) ).select( limitby=( start, limit) , orderby=orderby )
	#print db._lastsql
	for row in query:		
