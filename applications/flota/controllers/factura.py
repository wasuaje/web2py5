...
def index():   
	tabla='fc_documento'
	tabla2='cf_proveedor'
	data=((db[tabla].cf_proveedor_id==db.cf_proveedor.id) & (db[tabla].fc_tipo_doc_id==2)  &  (db[tabla].status==None))
...
def show_form():		
	...
	for i in range(0, len(request.args)):
		if request.args(i)=='True':
			thereistrue=True            
			funcion="/select/factura/insert_detail/"+request.args(0)
	if thereistrue ==  True:
		readonly=True
		deletable=False
	else:		
	...
	form=SQLFORM(tabladb, registro, submit_button='Enviar' ,    delete_label='Click para borrar:', next=URL(c='factura',  f='index') , \
			fields=campos ,showid=False , deletable=deletable ,  readonly=readonly , labels=labels, upload=URL('download'))	
	rectipdoc=db(db.fc_tipo_doc.nombre.like('%Factura%')).select()
	...
	if form.accepts(request.vars, session):
		...
		if form.vars.delete_this_record=="on":
			...
			redirect(URL(c='factura',  f='index'))
		else:
			redirect(URL(c='factura',  f='show_form/'+str(myid)+'/True'))
	elif form.errors:
...
def insert_detail():
	...
	if inserted and inserted > 0:        
		...
		redirect(URL(c='factura',  f='show_form/'+document_id+'/True'))
...
def show_detail(elid):
    ...
    lineas = DIV(listado)
    pie=[]
    pie.append((TD(), TD('Total Factura: '),TD(str(total))))
    #print lineas
...
def imprimir():
	...
	f = Template(format="A4",
			title="factura", author="w.a",
			subject="w.a", keywords="")		
	f.parse_csv(infile=request.folder+'/static/rpt/factura.csv', delimiter=";", decimal_sep=",")
